...
import { resolveActiveSession, saveSession } from "../../../lib/auth/session";
...
export default function TeamPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(
        toErrorMessage(loadError, locale === "en" ? "Team settings could not be loaded." : "Ekip ayarları yüklenemedi.")
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleResendInvitation(userId: string) {
    ...
    try {
      await apiClient.resendMemberInvitation(userId);
      setMemberActionNotice(
        locale === "en"
    ...
    return (
      <section className="page-grid">
        <section className="panel">
          <LoadingState message={t("Ekip ayarları yükleniyor...")} />
        </section>
      </section>
    );
  }
    ...
    <section className="page-grid">
      <div className="page-header page-header-plain">
        <div className="page-header-copy">
          <PageTitleWithGuide guideKey="team" title={title} subtitle={subtitle} style={{ margin: 0 }} />
        </div>
      </div>
                ...
                {members.map((member) => {
                  ...
                  return (
                    <tr key={member.userId}>
                      ...
                      <td>{formatOptionalDate(member.lastLoginAt)}</td>
                      <td>
