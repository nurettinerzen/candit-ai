...
export default function AiSupportCenterPage() {
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "AI destek merkezi yüklenemedi.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function toggleFlag(flag: FeatureFlag, nextValue: boolean) {
    ...
    try {
      ...
      setError(toggleError instanceof Error ? toggleError.message : "Feature flag güncellenemedi.");
    } finally {
      setSavingFlagKey("");
    }
  }
  ...
  return (
    <section className="panel">
      ...
      {loading ? <LoadingState message="AI destek merkezi yükleniyor..." /> : null}
      {!loading && error ? (
        <ErrorState
          error={error}
          actions={
        ...
        <>
          <div className="mini-grid">
            <article className="panel nested-panel">
              ...
              {providers.length === 0 ? (
                <EmptyState message="Sağlayıcı bulunamadı." />
              ) : (
          ...
          <section className="panel nested-panel" style={{ marginTop: 16 }}>
            ...
            {demoFlags.length === 0 ? (
              <EmptyState message="Demo flag kaydı bulunamadı." />
            ) : (
              <table className="table">
                <thead>
                  <tr>
                        ...
                        <td>
                          {canToggle ? (
                            <button
                              ...
                              disabled={savingFlagKey === flag.key || flag.key === "ai.auto_reject.enabled"}
          ...
          <section className="panel nested-panel" style={{ marginTop: 16 }}>
            ...
            {taskRuns.length === 0 ? (
              <EmptyState message="AI task run kaydı bulunamadı." />
            ) : (
              <table className="table">
                <thead>
                  <tr>
          ...
          <section className="panel nested-panel" style={{ marginTop: 16 }}>
            ...
            {infrastructure?.integrations && infrastructure.integrations.length > 0 ? (
              ...
              <EmptyState message="Integration connection kaydı yok." />
            )}
          </section>
        </>
      ) : null}
