...
export const metadata: Metadata = {
  ...
  icons: {
    icon: [
      { url: BRAND_THEME_ASSETS.light.faviconPng, type: "image/png", sizes: "512x512" },
...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <html
      ...
      style={{
        backgroundColor: DOCUMENT_THEME_COLORS[initialResolvedTheme].background,
      ...
      <head>
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){try{var d=document.documentElement;function c(name){var m=document.cookie.match(new RegExp('(?:^|; )'+name.replace(/([.$?*|{}()\\[\\]\\\\/+^])/g,'\\\\$1')+'=([^;]*)'));return m?decodeURIComponent(m[1]):null}function u(id,rel,href,type,sizes){var h=document.head;if(!h)return;var link=document.getElementById(id);if(!link){link=document.createElement('link');link.id=id;h.appendChild(link)}link.rel=rel;link.href=href;if(type){link.type=type}else{link.removeAttribute('type')}if(sizes){link.sizes=sizes}else{link.removeAttribute('sizes')}}function m(content){var h=document.head;if(!h)return;var meta=document.getElementById('brand-theme-color');if(!meta){meta=document.createElement('meta');meta.id='brand-theme-color';meta.name='theme-color';h.appendChild(meta)}meta.content=content}var a={light:{png:'${BRAND_THEME_ASSETS.light.faviconPng}',ico:'${BRAND_THEME_ASSETS.light.faviconIco}',apple:'${BRAND_THEME_ASSETS.light.appleTouch}',themeColor:'${BRAND_THEME_ASSETS.light.themeColor}',background:'${DOCUMENT_THEME_COLORS.light.background}'},dark:{png:'${BRAND_THEME_ASSETS.dark.faviconPng}',ico:'${BRAND_THEME_ASSETS.dark.faviconIco}',apple:'${BRAND_THEME_ASSETS.dark.appleTouch}',themeColor:'${BRAND_THEME_ASSETS.dark.themeColor}',background:'${DOCUMENT_THEME_COLORS.dark.background}'}};var t=d.getAttribute("data-theme-mode")||c("ai_interviewer_theme")||localStorage.getItem("ai_interviewer_theme")||"system";if(t!=="light"&&t!=="dark"&&t!=="system")t="system";var r=d.getAttribute("data-theme")||c("ai_interviewer_theme_resolved");if(r!=="light"&&r!=="dark"){r=t==="light"||t==="dark"?t:(window.matchMedia("(prefers-color-scheme:dark)").matches?"dark":"light")}d.setAttribute("data-theme",r);d.setAttribute("data-theme-mode",t);var s=a[r==="dark"?"dark":"light"];d.style.backgroundColor=s.background;d.style.colorScheme=r;var l=d.getAttribute("data-locale")||c("ai_interviewer_site_locale")||localStorage.getItem("ai_interviewer_site_locale")||"tr";if(l!=="tr"&&l!=="en")l="tr";d.setAttribute("data-locale",l);d.setAttribute("data-locale-ready","true");d.lang=l;document.cookie="ai_interviewer_theme="+t+"; Path=/; Max-Age=31536000; SameSite=Lax";document.cookie="ai_interviewer_theme_resolved="+r+"; Path=/; Max-Age=31536000; SameSite=Lax";document.cookie="ai_interviewer_site_locale="+l+"; Path=/; Max-Age=31536000; SameSite=Lax";u('brand-favicon-png','icon',s.png,'image/png','512x512');u('brand-favicon-ico','shortcut icon',s.ico,'image/x-icon');u('brand-apple-touch','apple-touch-icon',s.apple,'image/png','180x180');m(s.themeColor);try{localStorage.setItem("ai_interviewer_theme",t);localStorage.setItem("ai_interviewer_site_locale",l)}catch(e){}}catch(e){}})()`,
          }}
        />
      </head>
      <body>
