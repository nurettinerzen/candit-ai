...
test("public lead notifications fall back to the first internal admin inbox when explicit recipients are missing", () => {
  const runtimeConfig = createRuntimeConfig({
    INTERNAL_ADMIN_EMAIL_ALLOWLIST: "ops@candit.ai,team@candit.ai"
  });

  assert.deepEqual(runtimeConfig.publicLeadNotificationRecipients, ["ops@candit.ai"]);
  assert.ok(
    runtimeConfig
...
test("configured public lead recipients suppress the fallback warning", () => {
  const runtimeConfig = createRuntimeConfig({
